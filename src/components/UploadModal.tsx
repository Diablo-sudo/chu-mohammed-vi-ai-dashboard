  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2 text-[var(--color-chu-text)]">⚙️ {t.updateData}</h2>
        <div className="space-y-4 mb-6">
          <div>
            <label className="block text-xs font-bold text-[var(--color-chu-text-sec)] uppercase mb-1">{t.admissionsCsv}</label>
          ...
          <div>
            <label className="block text-xs font-bold text-[var(--color-chu-text-sec)] uppercase mb-1">{t.patientsCsv}</label>
            <input type="file" accept=".csv" onChange={(e) => setUploadFiles(prev => ({...prev, patients: e.target.files?.[0] || null}))}
              className="w-full text-sm text-[var(--color-chu-text)] file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[#00D4AA]/10 file:text-[#00D4AA] hover:file:bg-[#00D4AA]/20" />
          </div>
          <div>
            <label className="block text-xs font-bold text-[var(--color-chu-text-sec)] uppercase mb-1">{t.diagnosesCsv}</label>
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={() => setShowUploadModal(false)}
            className={`flex-1 py-2.5 rounded-xl font-bold text-sm transition-colors ${!darkMode ? 'bg-gray-100 text-gray-700 hover:bg-gray-200' : 'bg-[#1B2A4A] text-white hover:bg-[#2A3F5F]'}`}>{t.cancel}</button>
